...
2014年の本なので少し古い部分もあるが、言語自体がナーフされた訳じゃないので2020年現在でも通用しそうな知識は多い。

最後にWebComponentsについて触れられている項目が現在のReActやVue.jsに相当するのだろうか。
...
各種サンプルを交えながらケース別にCSSの良い例と悪い例を紹介している。
...
プリプロセッサは、現状のCSSが持っていないプログラミング言語のような機能を与えたり、スタイルを簡潔に書けたりするようにするための機能、分割したファイルの管理などを可能にする。  
[Sass](http://sass-lang.com/guide)を使った管理方法がスタンダードらしい。

ファイルは分割して管理したいが、読み込むファイル数がページの表示速度に影響するという問題を、複数ファイルで管理して成果物は1ファイルにできるプリプロセッサが解決してくれる。
...
DOMとcssをセットでカプセル化したり、独自のタグを定義してtemplateで管理したりする構想の話。  
2014年に登場したてだったとして、2020年現在はもう実現していそうな話だ。素晴らしい。
